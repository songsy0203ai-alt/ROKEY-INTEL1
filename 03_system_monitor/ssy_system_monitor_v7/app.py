...
class FireCameraWorker:
    def __init__(
        ...
        sound_player: AlarmSoundPlayer = "/home/rokey/ssy_ws/03_system_monitor/ssy_system_monitor_v7/Fire.wav",   # ✅ 추가
    ):
...
def receive_log():
    try:
        ...
        if value is None:
            return jsonify({"result": "fail", "reason": "missing value"}), 400
        ...
        ulabel = label.upper()
        if ulabel in ["AMR1", "ROBOT2", "R2", "2"]:
            latest_gauges["AMR1"] = payload
        elif ulabel in ["AMR2", "ROBOT3", "R3", "3"]:
            latest_gauges["AMR2"] = payload
        else:
            latest_gauges[label] = payload

        db_log(f"GAUGE_{label}_{status}", value_f)

        gauge_logs.append(payload)
        if len(gauge_logs) > GAUGE_LOG_MAXLEN:
            del gauge_logs[:-GAUGE_LOG_MAXLEN]

        print(f"[GAUGE /log] {payload}")
        return jsonify({"result": "success"}), 200

    except Exception as e:
        return jsonify({"result": "fail", "error": str(e)}), 500

@app.route("/api/gauge_logs")
def api_gauge_logs():
    return jsonify(gauge_logs[-50:])
...
def main():
    ...
    sound_player = AlarmSoundPlayer(
        wav_path="/home/rokey/ssy_ws/03_system_monitor/ssy_system_monitor_v7/Fire.wav",
        interval_sec=22.0
    )
