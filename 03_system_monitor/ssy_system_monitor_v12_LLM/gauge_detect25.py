...
class GaugeMonitorNode(Node):
    ...
    def arrival_callback(self, msg):
        if msg.data:
            # GATEKEEPER: If we are already waiting or already monitoring, IGNORE the signal
            if self.is_waiting_for_stabilization or self.monitoring_active:
                return 

            self.get_logger().info('>>> Waypoint arrived. AMR settling (2s)...')
            self.is_waiting_for_stabilization = True # Lock the gate
            self.monitoring_active = False 
            self.is_calibrated = False
            self.value_history.clear()
            
            # Start a one-shot timer
            self.create_timer(2.0, self.activate_monitoring)
        else:
            self.get_logger().warn('Navigation ended. Shutting down node.')
            raise SystemExit

    def activate_monitoring(self):
        self.get_logger().info('>>> Status: STABILIZED. Pipeline started.')
        self.monitoring_active = True
        self.is_waiting_for_stabilization = False # Unlock the gate
    ...
    def check_safety(self, value):
        is_safe = 1.0 < value < 8.0
        msg = Bool(data=is_safe)
        
        # 상태가 변했을 때만 (또는 처음 판단했을 때만) Flask로 전송
        if is_safe != self.last_sent_status:
            try:
                # Flask 서버 주소 (라벨, 수치, 상태 포함)
                log_data = {
                    "label": self.current_gauge_label,
                    "value": round(value, 2),
                    "status": "정상" if is_safe else "비정상"
                }
                requests.post("http://192.168.108.21:5000/", json=log_data, timeout=0.5)
                self.last_sent_status = is_safe # 현재 상태 저장
            except Exception as e:
                self.get_logger().error(f'Flask 전송 실패: {e}')

        # 기존 로직 유지
        if is_safe:
            self.get_logger().info(f'[SAFE] {value:.2f} 전송 후 대기.')
            self.ready_pub.publish(msg)
            self.monitoring_active = False 
        else:
            self.get_logger().warn(f'[UNSAFE] {value:.2f} 경보 발생.')
            self.ready_pub.publish(msg)
